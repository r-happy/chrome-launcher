...
async function loadCurrentTabs() {
  try {
    ...
    return tabs.filter((tab) => tab.url && tab.title).map((tab) => ({
...
function updatePreview() {
  if (!previewContainer || filteredSites.length === 0)
    return;
  const selectedSite = filteredSites[selectedIndex];
  if (!selectedSite)
    return;
  const existingTab = selectedSite.type !== "tab" ? findExistingTab(selectedSite.url) : null;
  let statusInfo = "";
...
function createLinkElement(site, index) {
...
function findExistingTab(url) {
  return allSites.find((site) => site.type === "tab" && site.url === url && site.tabId) || null;
}
